...
# Function to load data from gold_aggregation_of_sales_by_seller
def load_data_gold_aggregation_of_sales_by_seller():
    conn = connect_to_postgre()
    query = "SELECT * FROM gold_aggregation_of_sales_by_seller;"
    with conn.cursor() as cursor:
        cursor.execute(query)
        data = cursor.fetchall()
        columns = [desc[0] for desc in cursor.description]
...
# Function to load data from gold_last_seven_days
def load_data_gold_last_seven_days():
    conn = connect_to_postgre()
    query = "SELECT * FROM gold_last_seven_days;"
    with conn.cursor() as cursor:
        cursor.execute(query)
        data = cursor.fetchall()
        columns = [desc[0] for desc in cursor.description]
...
if __name__ == "__main__":
    ...
    '''
    sales_data_by_seller = load_data_gold_aggregation_of_sales_by_seller()
    save_in_json(sales_data_by_seller, "gold_aggregation_of_sales_by_seller.json")

    sales_data_by_product = load_data_gold_last_seven_days()
    save_in_json(sales_data_by_product, "gold_last_seven_days.json")
